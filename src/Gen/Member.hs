...
memberGen md (PM.MFun fd stat) = do
    ...
    let x = GF.emptyFuncGenData md (md ^. GL.functions) (fd ^. PM.params)
    let (et, s) =
            runState (GE.initGen >> GE.exprGen stat >> GE.exprType stat) x
    ...
    codeSection
        %= (`D.snoc` WA.FunctionBody
               ((map (WA.LocalEntry 1) . D.toList . (^. GF.locals)) s)
               ((D.toList . opCodeF . (^. GF.opCodes)) s)
           )
    return ()
